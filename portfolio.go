...
func projectIndex(files []os.DirEntry, path string) []DirLink {
	...
	for _, v := range files {
		...
		newProject := DirLink{URL: filepath.Join("/", path, v.Name()), Text: v.Name()}
...
func highlightCode(codeBlock Code) (highlitedCodeBlock Code, err error) {
	...
	formatter := html.New(html.Standalone(true) ,html.WrapLongLines(true), html.WithLineNumbers(true), html.TabWidth(2))
	...
	highlitedCodeBlock = Code{Content: highlitedCode.String(), Filename: codeBlock.Filename}
	return
}
